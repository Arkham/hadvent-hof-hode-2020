...
main = do
  ...
  let rows = map parseLine (T.lines contents)
  print $ findTreesForPattern rows (3, 1)
  let patterns = [(1, 1), (3, 1), (5, 1), (7, 1), (1, 2)]
  print $ product $ map (findTreesForPattern rows) patterns
...
takeOneEvery 1 list == [1, 2, 3, 4, 5]
takeOneEvery 2 list == [1, 3, 5]
takeOneEvery 3 list == [1, 4]
-}
takeOneEvery :: Int -> [a] -> [a]
takeOneEvery interval collection =
  case collection of
    [] -> []
    (first : rest) ->
